  ...
  const checkExistingUser = async (phoneNumber: string) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/auth/check/user`,
    ...
    if (confirmationResult && otp) {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/auth/verify-phone`,
          {
            method: "POST",
            headers: {
                        ...
                        <div className="w-full sm:w-2/3 space-y-3">
                          <div>
                            <div className="flex justify-between items-center">
                              <label
                                htmlFor="name"
                                className="text-sm font-medium text-gray-300 font-[Helvetica]"
                              >
                                NAME *
                              </label>
                              {nameError && (
                                <p className="text-xs text-red-500 uppercase font-serif ">
                                  {nameError}
                                </p>
                              )}
                            </div>
