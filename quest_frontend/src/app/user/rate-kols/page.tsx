...
const Profile: React.FC<ProfileProps> = ({ _id, displayName, bio, image,createdQuests,followers,twitterInfo,teleInfo,discordInfo }) => {
  return (
    <>
      <div className="cursor-pointer" >
        <div className="group flex flex-col gap-2 sm:flex-row border-[#333333] border p-4 w-full max-w-md bg-black text-white m-auto my-5 rounded-lg shadow-xl hover:bg-[#8c71ff] hover:text-[#000000]">
          <div className="w-full sm:w-1/3 flex flex-col gap-2 items-center mb-4 sm:mb-0 justify-start">
            <Link href={`/user/profile/${_id}`} className="h_image-container w-28 h-28 overflow-hidden rounded-sm">
              <img
                src={image}
                alt={displayName}
            ...
            <div className="flex">
            {/* twitter */}
              <Link target="_blank" href={`${twitterInfo?.username ? `https://x.com/${twitterInfo?.username}`:"#"}`} className="box1 right-trapezium w-[2rem] h-[2rem] p-[1px] bg-zinc-800 ">
                <svg
              ...
              {/* discord */}
              <Link target="_blank" href={`${discordInfo?.discordId?`https://discord.com/channels/@me/${discordInfo?.discordId}`:"#"}`} className="box1 left-right-trapezium w-[2rem] h-[2rem] p-[1px] bg-zinc-800">
                <svg
              ...
              {/* telegram */}
              <Link target="_blank" href={`${teleInfo?.telegramId ? `https://web.telegram.org/a/#${teleInfo?.telegramId}`:"#"}`} className="box1 left-trapezium w-[2rem] h-[2rem] p-[1px] bg-zinc-800">
                <svg
                  className="box2 left-trapezium bg-black p-2"
                  xmlns="http://www.w3.org/2000/svg"
                  width="15"
                  height="15"
                  viewBox="0 0 15 15"
          ...
          <div className="w-full sm:w-2/3 flex flex-col sm:pl-4 p-2">
              <Link href={`/user/profile/${_id}`} className="text-sm font-bold text-start mr-7 mt-3 sm:text-left" >
                  {displayName}
              </Link>
               <div className="text-start sm:text-left">
              <span className='text-sm' >Bio: </span>
              <span className="text-xs opacity-70 text-white font-semibold justify-center">
                {bio && truncateToNWords(bio, 20)}
              </span>
            </div>
              <div className="flex flex-row justify-end items-center gap-2">
                <div className="flex justify-start items-baseline gap-1">
                  <span className='text-center text-md '>{ createdQuests?.length || '0' }</span>
                  <span className="opacity-40 text-xs">QUESTS</span>
                </div>
                <div className="flex justify-start items-baseline gap-1">
                  <span className='text-center text-md'>{ followers?.length || '0' }</span>
                  <span className="opacity-40 text-xs">FOLLOWERS</span>
                </div>
              </div>
