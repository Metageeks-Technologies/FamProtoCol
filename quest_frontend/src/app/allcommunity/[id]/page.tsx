...
import { fetchCommunity, joinCommunity } from "@/redux/reducer/communitySlice";
import { useRouter } from "next/navigation";
import { notify } from "@/utils/notify";
import ReferralForm from "@/app/components/referalPopUp";

export default function CommunityProject({ params }: { params: { id: any } }) {
  const columns = [
    { name: "NAME", uid: "name" },
              ...
              <div className="box1 community-clip p-[1px] bg-zinc-800">
                ...
                >
                  <div className="absolute lg:bottom-5 lg:right-5 sm:bottom-5 sm:right-5  bottom-1 right-1 flex gap-2 place-items-center ">
                    <div className="flex flex-col items-end justify-end gap-2 sm:gap-4 sm:flex-row">
                ...
                <div className="flex flex-col gap-5 w-full">
                  ...
                  {memberId && !userData?.includes(memberId) ? (
                    <>
                      {" "}
                      <button
                        className="bg-[#6e00fa] text-white lg:py-3 lg:px-4 sm:py-3 py-1 rounded-md lg:mt-2"
                        onClick={() => {
                          joinningCommunity(community);
                        }}
                      >
                        Join the community
                      </button>
            ...
            {currentQuests?.map((quest: any, index: number) => (
              <div
                ...
                onClick={(e) => {
                  ...
                  } else if (memberId && !userData?.includes(memberId)) {
