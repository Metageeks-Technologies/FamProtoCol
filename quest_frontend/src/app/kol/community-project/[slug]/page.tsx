...
export default function CommunityProject({
  ...
  const getUsers = useCallback(async () => {
    ...
    try {
      ...
      const sortedUsers = data?.friends.sort(
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${process.env.NEXT_PUBLIC_SERVER_URL}/community/${id}`
          );
          ...
          notify(
            "error",
            "There is some server error in deleting the community"
          );
        }
      }
    });
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="shadow-lg overflow-hidden">
            ...
            <div className="lg:w-48 lg:h-48 sm:h-40 sm:w-44 h-20 w-28  ">
              <div className="box1 right-trapezium p-[1px] bg-zinc-800 ">
                <img
                  src={community.logo}
                  alt={community.title}
                  className="box2 right-trapezium lg:w-48 lg:h-48 sm:h-40 sm:w-44 h-20 w-28  object-cover"
                />
              </div>
            </div>
          ...
          <div className="flex justify-end gap-2 items-center mb-4">
            <button
              ...
              className="px-5 py-2 bg-famViolate font-medium text-white sm:text-md rounded-full hover:bg-famViolate-light transition duration-300"
            >
              Create Quest
            </button>
            <button
              className="bg-gray-900 hover:bg-gray-700 text-white font-medium  px-5 py-2 rounded-full"
              onClick={() => router.back()}
            >
              Go Back
            </button>
        ...
        <div className="mai div flex  lg:flex-row sm:flex-col flex-col mt-16 gap-24 justify-end ">
          <section className="w-full flex flex-col justify-center items-center">
            ...
            <div className="w-full lg:w-[80%] flex userTable justify-center items-center">
        ...
        <div className="p-2 edu mt-10 grid lg:gap-10 sm:gap-10 gap-4 sm:mx-8 mx-0 lg:mx-0 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          {currentQuests?.map((quest: any, index: number) => (
            ...
            >
              <div>
                <div
                  className={`box1 education-clip p-[1px] ${
                    index % 4 == 0 && "bg-red-500"
                  } ${index % 4 == 1 && "bg-[#4E46D6]"} ${
                    index % 4 == 2 && "bg-green-500"
                  } ${index % 4 == 3 && "bg-yellow-500"} `}
                >
                  <div className="education-clip box2 h-20 w-40 bg-black flex justify-center items-center p-4">
                ...
                <div className="mt-2 flex gap-3 justify-center">
                  ...
                  <div className="text-small text-slate-300">{quest.title}</div>
                </div>
              </div>
            </div>
          ))}
