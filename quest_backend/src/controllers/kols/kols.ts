...
export const createKols = async (
  ...
  try {
    ...
    if (!req.body.kolsData) {
      return res
        .status(400)
        .json({ error: "Bad Request", message: "No KolsData provided" });
    }
    const {
      name,
      userName,
    ...
    if (!name || !userName || !role) {
      return res
        ...
        .json({
          error: "Bad Request",
          message: "Missing required KolsData fields",
        });
    }
    ...
    if (existingKolsData) {
      return res.status(409).json({
        error: "Conflict",
        message:
          "KolsData with the same name, userName, and role already exists",
      });
    }
...
export const getAllKol = async (req: Request, res: Response) => {
  try {
    const kols =await UserDb.find({
      $expr: {
        $gt: [{ $size: "$createdCommunities" }, 0],
      },
    });
    // console.log("KolsData:", kols);
    return res.status(200).json({
      success: true,
...
export const updateKol = async (req: Request, res: Response) => {
  try {
    ...
    const kols = await KolsDB.findByIdAndUpdate(
      id,
      { name, userName, role, bio, imageUrl, upVotes, downVotes, socialLinks },
      { new: true }
    );
    return res.status(200).json({
      success: true,
