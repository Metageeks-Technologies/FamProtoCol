...
export const fetchGuildChannel = async (req: Request, res: Response) => {
...
export const checkUserGuilds = async (req: Request, res: Response) => {
  if (!req.user) {
    return res
      .status(201)
...
export const inviteLink = async (req: Request, res: Response) => {
  ...
  const userExist = await User.findById(user.ids);
  if (!userExist) {
